...
class AdversarialLoss(nn.Module):
    ...
    def train_adv(self, fake: torch.Tensor, real: torch.Tensor) -> torch.Tensor:
        ...
        This will automatically synchronize gradients and call the optimizer.
        """
...
def get_fake_criterion(loss_type: str) -> tp.Callable:
    """Get fake loss criterion for discriminator training."""
    assert loss_type in ADVERSARIAL_LOSSES
    if loss_type == 'mse':
...
def get_real_criterion(loss_type: str) -> tp.Callable:
    """Get real loss criterion for discriminator training."""
    assert loss_type in ADVERSARIAL_LOSSES
    if loss_type == 'mse':
...
def mse_real_loss(x: torch.Tensor) -> torch.Tensor:
    """MSE loss for real samples (discriminator should output 1)."""
    return F.mse_loss(x, torch.tensor(1., device=x.device).expand_as(x))


def mse_fake_loss(x: torch.Tensor) -> torch.Tensor:
    """MSE loss for fake samples (discriminator should output 0)."""
...
def mse_loss(x: torch.Tensor) -> torch.Tensor:
    """MSE loss for generator training (should fool discriminator to output 1)."""
    if x.numel() == 0:
...
def hinge2_loss(x: torch.Tensor) -> torch.Tensor:
    """Hinge2 loss for generator training (should fool discriminator to output > 1)."""
    if x.numel() == 0:
        return torch.tensor([0.0], device=x.device)
    return -torch.mean(torch.min(x - 1, torch.tensor(0., device=x.device).expand_as(x)))
...
class FeatureMatchingLoss(nn.Module):
    ...
    Args:
        ...
        normalize (bool): Whether to normalize the loss by number of feature maps.
    """
...
# Convenience function to create adversarial loss with MS-STFT discriminator
def create_adversarial_loss(discriminator: nn.Module, 
                           optimizer: torch.optim.Optimizer,
                           loss_type: str = 'hinge',
                           use_feature_matching: bool = True) -> AdversarialLoss:
    """Create adversarial loss with MS-STFT discriminator.
    
    Args:
        discriminator: MS-STFT discriminator
        optimizer: Optimizer for discriminator
        loss_type: Type of adversarial loss ('mse', 'hinge', 'hinge2')
        use_feature_matching: Whether to use feature matching loss
        
    Returns:
        AdversarialLoss instance
    """
    # Get loss functions
    adv_loss = get_adv_criterion(loss_type)
    real_loss = get_real_criterion(loss_type)
    fake_loss = get_fake_criterion(loss_type)
    
    # Feature matching loss
    feat_loss = FeatureMatchingLoss() if use_feature_matching else None
