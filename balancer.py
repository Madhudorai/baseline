...
class Balancer:
    ...
    Args:
        ...
        ema_decay (float): EMA decay for averaging the norms.
    ...
    def __init__(self, weights: tp.Dict[str, float], balance_grads: bool = True, total_norm: float = 1.,
        ...
        self.averager = self._create_averager(ema_decay or 1.)
    ...
    def _create_averager(self, ema_decay: float):
        """Create a simple exponential moving average tracker."""
            ...
            def __call__(self, metrics):
                for key, value in metrics.items():
                    if key not in self.state:
                        ...
                        self.state[key] = self.decay * self.state[key] + (1 - self.decay) * value
