...
from utils import pad_for_conv1d
...
class ReconstructionLoss(nn.Module):
    """Combined reconstruction loss with time and frequency domain terms.
    
    This implements the loss from the EnCodec paper:
    - Time domain: L1 loss between target and compressed audio
    - Frequency domain: Multi-scale mel-spectrogram loss with L1 + L2 terms
    ...
    """
    def __init__(self, sample_rate: int, time_weight: float = 1.0, freq_weight: float = 1.0):
        super().__init__()
        ...
        # Frequency domain: Multi-scale mel-spectrogram loss
        # Using scales 2^5 to 2^11 (32 to 2048) with 64 mel bins
        self.freq_loss = MultiScaleMelSpectrogramLoss(
            sample_rate=sample_rate,
            range_start=5,  # 2^5 = 32
            range_end=12,   # 2^11 = 2048
            n_mels=64,
            normalized=True,
            alphas=True,    # Use αi = √(2^i - 1) as in the paper
            floor_level=1e-5
        )
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: Predicted/reconstructed audio [B, C, T]
            y: Target audio [B, C, T]
            
        Returns:
            Combined reconstruction loss
        """
        # Time domain loss: ||x - x̂||₁
        time_loss = self.time_loss(x, y)
        
        # Frequency domain loss: Multi-scale mel-spectrogram
        freq_loss = self.freq_loss(x, y)
        
        # Combined loss: λ_t * time_loss + λ_f * freq_loss
